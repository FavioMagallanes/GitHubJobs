...
function validarBusqueda(e) {
  ...
  if (busqueda === '') {
    mostrarMensaje('Debes ingresar una busqueda');
    return;
  }
  ...
  if (vacantes.length > 0) {
    ...
    vacantes.forEach(vacante => {
      ...
      <div class="shadow bg-white p-6 rounded">
        <h2 class="text-3l font-bold mb-4">${title}</h2>
        <p class="font-bold uppercase">Empresa:  <span class="font-light">${company} </span></p>
        <p class="font-bold uppercase">Modalidad: <span class="font-light">${type} </span></p>
